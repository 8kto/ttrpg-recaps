...
- Вес инвентаря нужно посчитать, и вывести свой рейтинг передвижения Base Movement Rate (стр. 32). По нему мы узнаём
  скорость персонажа. В рулбуке не у всех предметов есть вес, поэтому берём его из [этих таблиц](https://oldschoolreactionary.blogspot.com/2017/02/swords-wizardry-house-rules-encumbrance.html).
...
Для S&W Complete/Revised нет генераторов персонажей, поэтому можно взять один из тех, что под OD&D

- https://osr.smolderingwizard.com/osr/random_odd_char
- https://character.totalpartykill.ca/lbb/

Генераторы выше выводят _нисходящий AC_, который нужно сконвертировать в _восходящий (Ascending AC)_ по формуле
`AAC = 19 - DAC`. Например, нисходящий AC 2, значит восходящий AC = 17.

- Используем один спасбросок, его нужно взять из рулбука (а те, что в генераторе проигнорировать).
- Вес инвентаря посчитать, вывести Base Movement Rate
